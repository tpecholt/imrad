...
{
    const float BWIDTH = 80;
    const float BHEIGHT = 25;
    ...
    if (ImGui::BeginPopupModal((title + "###MessageBox").c_str(), &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        float height = std::max(25.f * dp, szm.y);
        if (icon != None) {
            ImGui::PushFont(nullptr, 20.f * dp);
            height = std::max(height, ImGui::GetFontSize());
        ...
        ImGui::BeginChild("ch", { WIDTH * dp, height + 3 * ImGui::GetStyle().ItemSpacing.y });
        ImGui::TextWrapped("%s", message.c_str());
        ImGui::EndChild();
        ImGui::Spacing();

        int n = (bool)(buttons & ImRad::Ok) + (bool)(buttons & ImRad::Cancel) + (bool)(buttons & ImRad::Yes) + (bool)(buttons & ImRad::No);
        float w = (n * BWIDTH*dp) + (n - 1) * ImGui::GetStyle().ItemSpacing.x;
        ...
        if (buttons & ImRad::Ok)
        {
            ...
            if (ImGui::Button("OK", { BWIDTH*dp, BHEIGHT*dp }) ||
        ...
        {
            if (ImGui::Button("Yes", { BWIDTH*dp, BHEIGHT*dp })) {
                ImGui::CloseCurrentPopup();
                callback(ImRad::Yes);
            }
            ImGui::SameLine();
        ...
        if (buttons & ImRad::No)
        {
            if (ImGui::Button("No", { BWIDTH*dp, BHEIGHT*dp })) {
                ImGui::CloseCurrentPopup();
                callback(ImRad::No);
            }
            ImGui::SameLine();
        ...
        {
            if (ImGui::Button("Cancel", { BWIDTH*dp, BHEIGHT*dp }) || ImGui::IsKeyPressed(ImGuiKey_Escape)) {
                ImGui::CloseCurrentPopup();
